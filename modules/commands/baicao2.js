...
module.exports.config = {
	...
	commandCategory: "game nhiều người chơi",
...
module.exports.handleReply = async function ({
  ...
  const { threadID, messageID } = event;
    ...
    if (event.senderID != author) return api.sendMessage("Ai dùng thì người đó reply =))", event.threadID, event.messageID); 
    switch (handleReply.type) {
    case "choosee": {
        switch (event.body) {
         case "help": {
          return api.sendMessage(`=== HELP === \nKhi bắt đầu ván đầu bot sẽ nhắn tin cho từng người, nếu mà không thấy tin nhắn thì nhắn tin cho bot rồi vào nhóm ghi đổi bài để bot lại`, event.threadID, event.messageID )}
}
    }
    }
              }